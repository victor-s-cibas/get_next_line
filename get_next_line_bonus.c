...
/*   Updated: 2026/07/12 14:12:22 by vicdos-s         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*   Updated: 2026/07/12 12:15:19 by vicdos-s         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static t_gnl_list	lists[1024];
	char				*line;

	if (fd < 0 || fd >= 1024 || BUFFER_SIZE <= 0)
		return (NULL);
	read_and_append(fd, &lists[fd]);
	if (!lists[fd].head)
		return (NULL);
	line = extract_line(&lists[fd]);
	update_list(&lists[fd]);
	return (line);
}
